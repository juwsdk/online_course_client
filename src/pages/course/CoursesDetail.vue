<template>
  <DetailView>
    <template v-slot:pageHeader>
      <el-page-header
        @back="goBack"
        :content="'课程页面' + $route.params.courseId"
      >
      </el-page-header>
    ...
    <template v-slot:pageAside>
      <el-aside
        ...
        v-show="tabsObj.activeTab == 'courseLearn'"
      >
        <el-menu class="el-menu-vertical-demo" style="">
          <el-menu-item
            v-for="courseRoute in courseRoutes"
            :key="courseRoute.courseResId"
            @click="showVedio(courseRoute.resFileName)"
            :disabled="isNotClickCourse"
          >
    ...
    <template v-slot:pageMain>
      ...
      <template v-if="tabsObj.activeTab == 'courseHomework'">
        <el-collapse @change="handleChange">
          <FileDownloadCard
            v-for="(homeworkItem, index) in homeworkList"
            :key="index"
            :index="index"
            :comptype="comptype"
        ...
        <CommentVeiw :showHeader="false">
          ...
          <template v-slot:commentsInput>
            <div id="timeBody">
              ...
              <div class="dateFormatNowStyle">模式:{{ inputMode }}</div>
              <div class="dateFormatNowStyle">
                <el-button
                  type="text"
                  v-show="inputMode == '回复'"
  ...
  methods: {
    ...
    handleClick(tab) {
      ...
      //如果是作业界面
        ...
        if (this.tabsObj.loadedData[1] == false) {
          //防止多次点击重复请求数据

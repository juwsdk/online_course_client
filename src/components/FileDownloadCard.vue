  ...
  <el-collapse-item :title="homeworkItem.courseHomeworkName" :name="index">
    <template>
      ...
      <template v-if="comptype=='student'">
        <el-button class="floatButton" type="text" @click="seeScore">查看得分</el-button>
        <el-button class="floatButton" type="text" @click="dialogVisible=true;">上传作业</el-button>
        <el-button class="floatButton" type="text" @click="dowloadItem">下载附件</el-button>
      </template>

      <!-- 如果父页面是教师组件的话 -->
      <template v-if="comptype=='teacher'">
        <el-button class="floatButton" type="text" @click="toViewIsFinish">查看学生完成情况</el-button>
      ...
      <!--   <el-popconfirm title="确定删除这个作业？" confirmButtonText="确定" cancelButtonText="取消" @confirm="deleteHomework">
        ...
        <el-drawer :title="homeworkItem.courseHomeworkName+'完成情况'" :visible.sync="drawer" direction="rtl"
          ...
          <article class="drawBody">
            <el-link class="articleN" type="success" v-for="(item,aindex) in studentSumbmitList" :key="aindex"
              @click="studentWhofinishWork(item)">{{item.studentName}}</el-link>
          </article>
          <div>
          ...
          <article>
            <el-link class="articleN" :underline="false" type="danger" v-for="(item,index) in studentNotSubmitList"
              :key="index">{{item}}</el-link>
          </article>
        </el-drawer>
        <!-- 教师下载学生完成作业的对话框 -->
        <el-dialog ref="teacherDialog" :title="studentSumbmitItem.studentName +'提交的作业'"
          :visible.sync="dialogVisibleTeacher" width="420px" :before-close="handleDialogClose">
          <div>
            <span>得分:</span>
            <el-input-number v-model="score" @change="handleChange" :min="0" :max="100"></el-input-number>
          </div>
          <el-button type="text" size="default" @click="downloadStudentHomework">下载附件</el-button>
          <span slot="footer">
            <el-button type="success" size="small" @click="scoreToStudent"
              style="float: left;margin-left: 100px;">评分</el-button>
            <el-button type="warning" size="small" @click="dialogVisibleTeacher=false"
              style="float: right; margin-right: 100px;">取消</el-button>
          </span>
        </el-dialog>
      </template>
    ...
    <!-- 上传作业的对话框 -->
    <el-dialog title="学生作业" :visible.sync="dialogVisible" width="420px" :before-close="handleDialogClose">
      <span>
    ...
    methods: {
      ...
      uploadItem() {
        this.fileList.forEach(fileItem => {
          ...
          //发送axios请求,上传数据
          axios.post("/homework/homeWorkFinishedUpload", formData, {
            headers: {
